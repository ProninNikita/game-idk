...
func _spawn_resource_clusters(resource_type: StringName, cluster_count: int, amount_per_cluster: int, radius: int) -> void:
	for i in range(cluster_count):
		var center := _random_grid_position()
...
func _spawn_cluster(resource_type: StringName, center: Vector2i, amount: int, radius: int) -> void:
	var spawned := 0
	var attempts := amount * 12

	while spawned < amount and attempts > 0:
		attempts -= 1
		var offset := Vector2i(_rng.randi_range(-radius, radius), _rng.randi_range(-radius, radius))
...
func _try_spawn_resource(resource_type: StringName, grid_pos: Vector2i) -> bool:
	...
	var node := RESOURCE_NODE_SCENE.instantiate() as HarvestableResourceNode
	node.setup(resource_type, grid_pos, tile_size, RESOURCE_DEFS[resource_type])
...
func _is_in_spawn_clearance(grid_pos: Vector2i) -> bool:
	var center := Vector2(float(map_size.x), float(map_size.y)) * 0.5
	var pos := Vector2(float(grid_pos.x), float(grid_pos.y))
...
func _draw() -> void:
	...
	var major_step := tile_size * 10
	for x in range(0, int(world_size.x) + 1, major_step):
		draw_line(Vector2(x, 0), Vector2(x, world_size.y), Color(0.08, 0.12, 0.09, 0.45), 1.0)
	for y in range(0, int(world_size.y) + 1, major_step):
